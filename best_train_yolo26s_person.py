# -*- coding: utf-8 -*-
"""
优化版 YOLOv26s 训练脚本 (带详细日志版) - 增加person类别
...
DATA_YAML = os.path.join(DATASET_DIR, "basketball_hd_dataset_person.yaml")
...
try:
    ...
    #     project="./runs/train",
    #     name="yolo26s_640_train_hd_person",  # 名字改一下
    #     exist_ok=True,
    #     patience=30,
    ...
    results = model.train(
        ...
        # --- 🛡️ [防崩溃优化] 针对 M3 MPS + 小目标 ---
        mosaic=1.0,      
        close_mosaic=10, 
        mixup=0.0,       # 🔴 必须关掉！半透明叠加极易让原本就很小的篮球彻底消失，导致张量报错
        scale=0.2,       # 🔴 从 0.5 降到 0.2！减少图片被大幅度缩小的概率，保住极小目标
        degrees=0.0,
        translate=0.1,
        fliplr=0.5,
        
        # --- ⚖️[抗不平衡优化] 针对人多球少 ---
        cls=3.0,         # 🌟 新增！分类损失权重 (默认0.5)。调高它，强迫模型在分类时更谨慎，避免把球错认成人
        # box=7.5,       # (可选) 边框损失权重，默认7.5，保持默认即可
        ...
        workers=8,
        project="./runs/train",
        name="yolo26s_640_train_hd_person",
        exist_ok=True,
        patience=30,
        save_period=5,
        verbose=True,
    ...
    traceback.print_exc() 
    print("-" * 40)
    print("💡 排错指南：请把上面最后几行涉及到 ultralytics 的报错发给 AI 分析。")
