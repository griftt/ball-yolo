...
VIDEO_PATH = "/Users/grifftwu/Desktop/历史篮球/20260116/test.mp4"
    ...
    while True:
        if not paused:
            ...
            if not ret:
                print("\n视频播放结束")
                print(
                    f"📊 检测统计: 总帧数={frame_count}, 篮筐检测帧={rim_detected_frames}, 篮球检测帧={ball_detected_frames}, 人员检测帧={person_detected_frames}"
                )
                break
            frame_count += 1
        else:
        ...
        results = model.predict(
            debug_frame, conf=0.01, device=device, verbose=False, imgsz=1024
        )

        rim_box = None
        has_ball_this_frame = False
        has_rim_this_frame = False
        has_person_this_frame = False
            ...
            for i, conf in enumerate(confs):
                ...
                if cls_id == 0:
                    if conf > CONF_THRES_BALL:
                        ...
                        print(
                            f"🥅 [帧{frame_count:05d}] {curr_time:.2f}s - 检测到篮球! 置信度={conf:.3f}, 位置=[{x1},{y1},{x2},{y2}]"
                        )
                    else:
                        ...
                        curr_time = cap.get(cv2.CAP_PROP_POS_FRAMES) / fps
                        print(
                            f"🥅 [帧{frame_count:05d}] {curr_time:.2f}s - 篮球置信度过低! 置信度={conf:.3f}, 位置=[{x1},{y1},{x2},{y2}]"
                        )
                ...
                # 🥅 篮筐
                    ...
                    if conf > CONF_THRES_RIM:
                        ...
                        if rim_box is None:
                            ...
                            curr_time = cap.get(cv2.CAP_PROP_POS_FRAMES) / fps
                            print(
                                f"🥅 [帧{frame_count:05d}] {curr_time:.2f}s - 检测到篮筐! 置信度={conf:.3f}, 位置=[{x1},{y1},{x2},{y2}]"
                            )
                    else:
                        # 打印低置信度的篮筐
                        if conf > 0.05:  # 只打印置信度>5%的
                            curr_time = cap.get(cv2.CAP_PROP_POS_FRAMES) / fps
                            print(
                                f"⚠️  [帧{frame_count:05d}] {curr_time:.2f}s - 篮筐置信度过低: {conf:.3f} < {CONF_THRES_RIM}"
                            )

                # 👤 人员
                elif cls_id == 2:
                    if conf > CONF_THRES_PERSON:
                        has_person_this_frame = True
                        color = (255, 0, 255)  # 紫色
                        ...
                        curr_time = cap.get(cv2.CAP_PROP_POS_FRAMES) / fps
                        print(
                            f"👤 [帧{frame_count:05d}] {curr_time:.2f}s - 检测到人员! 置信度={conf:.3f}, 位置=[{x1},{y1},{x2},{y2}]"
                        )
                    else:
                        color = (200, 200, 200)  # 灰色(被过滤)
                        cv2.rectangle(debug_frame, (x1, y1), (x2, y2), color, 1)
                        ...
                        curr_time = cap.get(cv2.CAP_PROP_POS_FRAMES) / fps
                        print(
                            f"👤 [帧{frame_count:05d}] {curr_time:.2f}s - 人员置信度过低! 置信度={conf:.3f}, 位置=[{x1},{y1},{x2},{y2}]"
                        )
        ...
        cv2.putText(
            small_frame,
            f"Rim: {rim_detected_frames}/{frame_count}",
            (20, 80),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.7,
            (0, 255, 0),
        ...
        cv2.putText(
            small_frame,
            f"Ball: {ball_detected_frames}/{frame_count}",
            (20, 110),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.7,
            (0, 140, 255),
        ...
        cv2.putText(
            small_frame,
            f"Person: {person_detected_frames}/{frame_count}",
            (20, 140),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.7,
            (255, 0, 255),
